...
<div id="mySidenav" class="sidenav">
    ...
    <div class="input-search-mb">
        <div class="div-search">
            <form method="POST" action="{{ url('/search') }}" accept-charset="utf-8">
                {{csrf_field()}}
                <input type="text" id="keyword" name="keyword" class="form-control input-search" placeholder="{{trans('toppage.Search')}}" value="@if(isset($keyword)){{$keyword}}@endif" required oninvalid="this.setCustomValidity('{{trans('toppage.can_not_empty')}}')">
    ...
    <div class="menu-side-right">
        @if( ! Auth::check() ) 
            <form method="POST" class="form-horizontal" role="form" accept-charset="utf-8" action="{{url('/account/login')}}" autocomplete="off">
                ...
                @endif
                 <div id="mb_alert_login_err" class="alert alert-danger">
